...
class ShaperHelper {
	private readonly _TestExpression: RegExp;
	private readonly _MatchExpression: RegExp;
	...
	private _adapter: object;

	public get adapter(): object {
		return this._adapter;
	}

	public set adapter(adapter) {
		this._adapter = adapter;
	}

	public static getInstance(): ShaperHelper {
		if (!ShaperHelper.self) {
			ShaperHelper.self = new ShaperHelper();
		}
		return ShaperHelper.self;
	}

	public _extractObject(object: object, keys: string[]): unknown | null {
	...
	public _extract(adapter, data, args): unknown {
		const self = this;
		return data.map(function (map) {
			...
			if (typeof adapter === 'function') {
				...
				if (adapter && (adapter.constructor === Object || adapter.constructor === Array)) {
					//todo check how adapter can possibilly be null or undefined
...
export const Shaper = ShaperHelper.getInstance();
